...
def get_blizzard_auth(client_id, secret):
  ...
  """
  r = requests.post(blizz_oauth_url, verify=False,
...
def get_hs_cards(token, search_term):
  ...
  r = requests.get(hs_search_url + search_term, headers=headers)
...
def main():
  ...
  async def on_message(message):
    ...
    if 'good bot' in message.content:
        await message.channel.send('https://tenor.com/view/thumbs-up-robot-good-gif-15533559')

    # bad bot
    if 'bad bot' in message.content:
        await message.channel.send('https://tenor.com/view/sad-r2d2-robot-beep-star-wars-gif-5812746')

    # roll
    if message.content.startswith('!roll'):
        roll_result = roll()
        await message.channel.send(f'{ message.author.name } rolls a { roll_result }')

    if message.content.startswith('!chucknorris'):
    ...
    if message.content.startswith('!mythic+'):
      await message.channel.send('Mythic + Rankings Here: ' + mythic_rankings_url)

    # raidbots sim your gear
    if message.content.startswith('!raidbots'):
      await message.channel.send('Sim your gear here: ' + raidbots_url)

    # class guides
    # death knight
    if message.content.startswith('!deathknight'):
      await message.channel.send(guide('blood-death-knight') + 
        guide('frost-death-knight') + guide('unholy-death-knight'))

    # demon hunter
    if message.content.startswith('!demonhunter'):
      await message.channel.send(guide('havoc-demon-hunter') + 
        guide('vengeance-demon-hunter'))

    # druid
    if message.content.startswith('!druid'):
      await message.channel.send(guide('balance-druid') + 
        guide('feral-druid') + guide('guardian-druid') + guide('restoration-druid'))

    # hunter
    if message.content.startswith('!hunter'):
      await message.channel.send(guide('beast-mastery-hunter') + 
        guide('marksmanship-hunter') + guide('survival-hunter'))

    # mage
    if message.content.startswith('!mage'):
      await message.channel.send(guide('arcane-mage') + 
        guide('fire-mage') + guide('frost-mage'))

    # monk
    if message.content.startswith('!monk'):
      await message.channel.send(guide('brewmaster-monk') + 
        guide('mistweaver-monk') + guide('windwalker-monk'))

    # paladin
    if message.content.startswith('!paladin'):
      await message.channel.send(guide('holy-paladin') + 
        guide('protection-paladin') + guide('retribution-paladin'))

    # priest
    if message.content.startswith('!priest'):
      await message.channel.send(guide('discipline-priest') + 
        guide('holy-priest') + guide('shadow-priest'))

    # rogue
    if message.content.startswith('!rogue'):
      await message.channel.send(guide('assassination-rogue') + 
        guide('outlaw-rogue') + guide('subtlety-rogue'))

    # shaman
    if message.content.startswith('!shaman'):
      await message.channel.send(guide('elemental-shaman') + 
        guide('enhancement-shaman') + guide('restoration-shaman'))

    # warlock
    if message.content.startswith('!warlock'):
      await message.channel.send(guide('affliction-warlock') + 
        guide('demonology-warlock') + guide('destruction-warlock'))

    # warrior
    if message.content.startswith('!warrior'):
      await message.channel.send(guide('arms-warrior') + 
        guide('fury-warrior') + guide('protection-warrior'))

    # hearthstone
    if message.content.startswith('!hearthstone'):
      ...
      if len(cards['cards']) > 8:
        await message.channel.send('Woah there! Too many results. Try a more specific search.')
        ...
        for i in range(0, len(cards['cards'])):
          ...
          if search_term.lower() in cards['cards'][i]['name'].lower():
            ...
            bot_message += 'Name: ' + cards['cards'][i]['name'] + '\n'

            if 'manaCost' in cards['cards'][i]:
              bot_message += 'Mana: ' + str(cards['cards'][i]['manaCost']) + '\n'

            if 'attack' in cards['cards'][i]:
              bot_message += 'ATK: ' + str(cards['cards'][i]['attack']) + '\n'

            if 'health' in cards['cards'][i]:
              bot_message += 'HP: ' + str(cards['cards'][i]['health']) + '\n'

            bot_message += 'Text: ' + cards['cards'][i]['text'] + '\n'
